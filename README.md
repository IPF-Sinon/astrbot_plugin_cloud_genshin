...
一个 AstrBot 插件，自动检测关键词并回复经典云原神梗。**v4.0 重磅升级**：支持多媒体发送（图片/语音/视频）+ 5种灵活回复模式！每组可独立配置文本梗段池 + 媒体池 + 回复模式，消息匹配到哪组就用哪组的配置回复，互不干扰。
...
| 🚫 **群聊黑/白名单** | 全局统一，所有组共享 |
| 📥 **v3.x/v2.x 自动迁移** | 旧版 data.json 自动迁移到 v4.0 格式 |
