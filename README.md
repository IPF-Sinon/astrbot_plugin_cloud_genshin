...
一个 AstrBot 插件，自动检测关键词并回复经典云原神梗。**v3.0 重磅升级**：支持多个匹配组，每组拥有独立的关键词列表、首次回复词、梗段词库和回复延迟！消息匹配到哪组就用哪组的配置回复，互不干扰。
...
| 功能 | 说明 |
|------|------|
| 🎯 **多匹配组** | 创建多个组，每组独立配置关键词、首次回复词、梗段词库、延迟 |
| 📦 **各组独立** | 消息匹配到对应组，回复该组专属内容，互不干扰 |
| 🎲 **梗段随机池** | 每组各配各的梗段池，每次随机取一段 |
| 👋 **先打招呼** | 各组独立配置首次回复词 |
| 💬 **手动命令** | `/云原神` / `/cloudys` 从第一组随机回复 |
| 🔧 **管理命令体系** | 丰富的 `group` 子命令管理多组：增删改查、命名、配置 |
| 🚫 **群聊黑/白名单** | 全局统一，所有组共享 |
| 📥 **v2.x 自动迁移** | 旧版 data.json 自动迁移到新版 match_groups 格式 |
...
- 关键词自动触发只对非命令消息生效
- **v2.x 用户首次升级**：data.json 会自动迁移到 v3.0 的 `match_groups` 格式
- 私聊不受黑/白名单影响
