...
### 🎨 配置设计正常化
- **修复了「首次安装初始值」的奇葩设计** 🥺：之前面板配置只在全新安装时读一次，之后改面板不生效，必须用管理命令。现在改为每次启动时自动同步面板配置到默认组，修改面板后重启即生效
- **新增 `/云原神管理 configsync` 命令**：运行时将面板配置同步到默认组，无需重启插件
- **新增 `_sync_config_to_data()` 方法**：管理命令修改默认组时自动保存，与面板配置双向同步
- **配置文件 `_conf_schema.json` 全面重构**：去掉了所有「仅首次安装使用」的迷惑描述，每个配置项清晰标注「默认组 · xxx」
