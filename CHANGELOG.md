...
### 🔧 版本更新 & 配置增强
- **版本号升级至 4.3.0**：跳过存在混淆的 v4.2.0 版本号，所有文件同步更新
- **保留已有 GitHub Release**：不再删除旧 Release，v4.2.0 Release 保留不动，仅创建新的 v4.3.0

### 🎨 全量组同步（接续 v4.2 重构）
- **面板 `match_groups` 全量管理所有组**：不再仅同步默认组，面板的 `match_groups` 数组可直接覆盖 `data.json` 中所有匹配组配置
- **独立字段改为「新建组默认」**：`trigger_keywords`、`first_reply`、`quote_pool` 等字段仅在新创建组时作为默认值，不影响已有组
- **启动时自动全量同步**：`_load_data()` 优先使用面板 `match_groups`（`copy.deepcopy`），降级为仅同步默认组
- **修复「首次安装初始值」迷惑设计** 🥺：面板配置现在实时生效，修改后重启或执行 `configsync` 即可同步

## [4.2.0] - 2026-06-14

### 🎨 配置设计正常化（已归档）
- 为 v4.3.0 的最终设计提供过渡，此版本未独立发布
