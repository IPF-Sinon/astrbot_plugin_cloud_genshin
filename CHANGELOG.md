...
- **`default_new_group` 替代散落字段**：不再有"新建组默认·xxx"这种黑话，而是一个完整的组对象模板，新建组时自动拷贝
- **`_make_default_group()` 改为从 `default_new_group` 读取**：面板模板优先，兜底硬编码默认值
- **删除降级同步逻辑**：`_load_data()` 和 `_sync_config_to_data()` 不再有散落字段的降级路径，面板没 `match_groups` 就直接跳过
- **面板配置描述清晰化**："新建组的默认模板：使用管理命令新建组时以此模板填充"——一眼看懂

## [4.3.0] - 2026-06-14

### 🔧 版本更新 & 配置增强（已归档）
- 为 v4.4.0 的最终简化提供过渡，此版本未独立发布
